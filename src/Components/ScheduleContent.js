...
const CourseTimeAndName = (props) => {
    return (
        <div style={{display:"block", textAlign:"left"}}>
            <h5 className="course-time-and-name" key={props.key}>{props.course.timeSlot + ' - '}</h5>
            <h5 className="course-time-and-name" key={props.key}><a className="a-schedule-item"href={"#course/"+props.course.courseNumber}>{props.course.title}</a></h5>
        </div>
    )
}